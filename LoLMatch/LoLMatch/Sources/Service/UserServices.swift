...
extension UserServices {
    ...
    private static func getPlayerKdaForThreeMatches(matches: [MatchBasicInfo], completion: @escaping ([FilteredMatch]?, Error?) -> Void) {
        ...
        for match in matches {
            ...
            getPlayerKdaForMatch(byGameId: gameId, forChampion: championId) { (filteredMatch, error) in
                ...
                let championCompletion = getChampionDetailsForMatch(match: filteredMatch)

                filteredMatches.append(championCompletion)
                dispatchGroup.leave()
            }
        }
    ...
    private static func getChampionDetailsForMatch(match: FilteredMatch) -> FilteredMatch {

        guard let championId = match.championId else { return match }

        let currentChampion = ChampionService.getChampion(by: championId)

        var newFilteredMatch = match
        newFilteredMatch.champion = currentChampion

        return newFilteredMatch
    }
}
